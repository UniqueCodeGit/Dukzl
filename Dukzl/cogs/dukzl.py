...
class DukzlCog(commands.Cog):
    ...
    async def register(self, ctx):
        ...
        embed = discord.Embed(
            title="축하합니다!", description="가입에 성공하였습니다. 이제 덕질봇 서비스를 이용하세요!", color=COLOR
        )
    ...
    async def myinfo(self, ctx):
        ...
        embed.add_field(name="서비스 가입 일자", value=data["started-date"], inline=False)
        embed.add_field(
            name="덕질하는 가수 수", value=f"{len(data['artists'])}명", inline=False
        )
        if len(data["artists"]) != 0:
            art = ", ".join(data["artistlist"][:])
            embed.add_field(name="덕질하는 가수", value=f"{art}", inline=False)
        ...
        try:
            ...
            for artists in data["artists"]:
                if artists["name"] == artist:
                    artistData = artists
                    break
            embed = discord.Embed(title=f"가수 **{artist}**의 덕질 정보 입니다.", color=COLOR)
            embed.add_field(
                name="덕질을 시작한 날", value=artistData["started-date"], inline=False
            )
            embed.add_field(
                name="덕질 레벨",
                value=f"{artistData['level']}exp - {round(float(artistData['level']) / 5, 1)} 레벨",
                inline=False,
            )
            embed.add_field(
                name="플레이리스트",
                value=f"{len(artistData['playlist'])} 곡 들어있음",
                inline=False,
            )
            await ctx.send(embed=embed)
        except KeyError:
    ...
    async def poppl(self, ctx, artist, *, url):
        ...
        if not self.Users.CheckArtistExists(ctx.author, artist):
            return await ctx.send(
                "해당 가수는 덕질하지 않습니다.  가수를 올바르게 입력했는지, URL이 올바른지 확인해주세요."
            )
        try:
    ...
    async def addinfo(self, ctx, *args):
        ...
        try:
            ...
            if not self.Users.CheckArtistExists(ctx.author, artist):
                return await ctx.send(
                    "해당 가수는 덕질하지 않습니다. 가수를 올바르게 입력했는지 확인해주세요. (띄어쓰기가 없어야 합니다.)"
                )
            ...
            if element == "생일":
                try:
                    ...
                    return await ctx.send(
                        "생일은 YYYY-MM-DD (예시 : 2020-01-01) 형식으로 입력해주세요."
                    )
